...
if exist "python\pythonw.exe" (
    ...
    python\python.exe login_script.py >> "%LOGFILE%" 2>&1
) else (
    echo [ERROR] Python not found in local directory! >> "%LOGFILE%"
    echo [INFO] Checking system Python... >> "%LOGFILE%"
    where pythonw.exe >nul 2>&1
    if errorlevel 1 (
        where python.exe >nul 2>&1
        if errorlevel 1 (
            echo [ERROR] No Python found in system PATH! >> "%LOGFILE%"
            exit /b 1
        ) else (
            python.exe login_script.py >> "%LOGFILE%" 2>&1
        )
    ) else (
        pythonw.exe login_script.py >> "%LOGFILE%" 2>&1
    )
)
"@
