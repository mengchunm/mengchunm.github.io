param(
    [string]$ResumeAction,
    [string]$ScriptSourceUrl = 'https://raw.githubusercontent.com/mengchunm/mengchunm.github.io/main/script.ps1'
)
...
try {
    ...
    Write-Verbose "无法设置控制台编码: $($_.Exception.Message)"
}

$script:SelfScriptBlock = $MyInvocation.MyCommand.ScriptBlock

$script:ScriptSourceUrl = $ScriptSourceUrl
$script:ResumeAction = $ResumeAction
...
function Install-AutoLogin {
    ...
    if (-not (Test-AdminRights)) {
        if (-not (Request-AdminElevation -ResumeAction 'InstallAutoLogin')) {
            Write-LogMessage '需要管理员权限才能安装自动登录脚本。' -Level 'WARNING'
            return
        }
    }
...
function AutoLogin-Menu {
    ...
    $choice = if ($PSBoundParameters.ContainsKey('InitialChoice') -and -not [string]::IsNullOrWhiteSpace($InitialChoice)) {
        Write-LogMessage "已根据管理员权限自动执行选项: $InitialChoice" -Level "INFO"
        $InitialChoice
    } else {
        Read-Host "��ѡ���Զ���¼���� (1-4, B����)"
    }
    ...
    switch ($choice) {
        ...
        "B" { return $false }
        default {
            Write-LogMessage "��Чѡ�����������롣" -Level "WARNING"
            Read-Host "�����������..." | Out-Null
        }
    }
    return $true
...
function Request-AdminElevation {
    ...
    Write-Host "注意：该操作需要管理员权限" -ForegroundColor Yellow
    Write-Host "继续后将以管理员身份重新启动脚本" -ForegroundColor Yellow
    Write-Host "========================================" -ForegroundColor Yellow

    $response = Read-Host "是否尝试管理员权限运行？(Y/N)"
    if ($response -notin @('Y', 'y')) {
        Write-LogMessage "保持当前权限继续运行，部分功能可能不可用。" -Level "WARNING"
        Start-Sleep -Seconds 2
        return $false
    }
    ...
    try {
        ...
        if ([string]::IsNullOrWhiteSpace($launchPath)) {
            if ([string]::IsNullOrWhiteSpace($script:ScriptSourceUrl)) {
                Write-LogMessage "无法确定脚本来源，无法自动提升权限。" -Level "ERROR"
                return $false
            }

            $tempPath = Join-Path ([System.IO.Path]::GetTempPath()) 'NetworkLoginScript.ps1'
            Invoke-WebRequest -Uri $script:ScriptSourceUrl -OutFile $tempPath -UseBasicParsing
            $launchPath = $tempPath
        }

        $argumentList += @('-File', $launchPath)

        if (-not [string]::IsNullOrWhiteSpace($script:ScriptSourceUrl)) {
            $argumentList += @('-ScriptSourceUrl', $script:ScriptSourceUrl)
        }

        if (-not [string]::IsNullOrWhiteSpace($ResumeAction)) {
            $argumentList += @('-ResumeAction', $ResumeAction)
        }

        Write-LogMessage "正在以管理员身份重新启动脚本..." -Level "INFO"
        Start-Process -FilePath 'powershell.exe' -ArgumentList $argumentList -Verb RunAs | Out-Null
        exit
    } catch {
        Write-LogMessage "管理员权限申请失败: $($_.Exception.Message)" -Level "ERROR"
        Start-Sleep -Seconds 2
        return $false
    }
}

function Invoke-AutoLoginManager {
    param(
        [string]$InitialChoice
    )

    $continueAutoLogin = $true
    $firstChoice = $InitialChoice
    while ($continueAutoLogin) {
        Show-Menu -MenuLevel "autologin" -Title "自动登录管理"
        if ($firstChoice) {
            $continueAutoLogin = AutoLogin-Menu -InitialChoice $firstChoice
            $firstChoice = $null
        } else {
...
function Company-Config {
    ...
    switch ($Choice) {
        1 {
            Invoke-AutoLoginManager
        }
        2 {
            Set-ProxySettings
        }
        default {
            Write-LogMessage "无效的公司配置选项。" -Level "WARNING"
...
if ($script:ResumeAction -eq 'InstallAutoLogin') {
    Write-LogMessage "检测到管理员复进，自动执行自动登录脚本安装。" -Level "INFO"
    Invoke-AutoLoginManager -InitialChoice '1'
    $script:ResumeAction = $null
}
...
do {
    switch ($currentMenu) {
        "main" {
            ...
            $choice = Read-Host "请选择一个选项 (1, 2, Q)"
            switch ($choice) {
                "1" { $currentMenu = "power" }
                ...
                "Q" { $script:exitScript = $true }
                default {
                    Write-LogMessage "无效的选项，请重新输入。" -Level "WARNING"
                    Read-Host "按任意键继续..." | Out-Null
                }
            }
        }
        ...
        "company" {
            Show-Menu -MenuLevel "company" -Title "公司配置"
            $choice = Read-Host "请选择公司配置 (1-2, B返回)"
            switch ($choice) {
