param(
    [switch]$DirectToAutoLogin,  # 直接跳转到自动登录安装
    [switch]$ElevatedSession     # 新增：标记这是提升权限后的会话
)
...
$script:scriptContent = $null  # 新增：保存脚本内容用于重新启动
...
function Install-AutoLogin {
    ...
    if (-not (Test-AdminRights)) {
        ...
        try {
            ...
            # 启动新的管理员进程，标记为提升会话并直接进入安装流程
            $arguments = "-ExecutionPolicy Bypass -File `"$tempScriptPath`" -DirectToAutoLogin -ElevatedSession"
